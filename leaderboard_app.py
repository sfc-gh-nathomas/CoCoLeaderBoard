...
def _setsail_ses_cte():
    """SEs: SE name joined to AMSExpansion accounts → region."""
    return f"""amsexp_ses AS (
    SELECT DISTINCT se.SALES_ENGINEER_NAME AS OWNER, o.REGION
    FROM SNOWPUBLIC.STREAMLIT.DIM_ACCOUNTS_SLIM_CACHE se
    JOIN SALES.RAVEN.SDA_OPPORTUNITY_VIEW o
         ON o.SALESFORCE_ACCOUNT_ID = se.ACCOUNT_ID
        AND o.THEATER = 'AMSExpansion'
        AND o.DS = {_OPP_MAX_DS}
    WHERE se.DS = (SELECT MAX(DS) FROM SNOWPUBLIC.STREAMLIT.DIM_ACCOUNTS_SLIM_CACHE)
      AND se.SALES_ENGINEER_NAME IS NOT NULL
    QUALIFY ROW_NUMBER() OVER (PARTITION BY se.SALES_ENGINEER_NAME
                               ORDER BY o.SALES_QUALIFIED_DATE DESC NULLS LAST) = 1
)"""
...
def _uc_params(t):
    return {"created":("CREATED_DATE::DATE",""), "won":("DECISION_DATE"," AND IS_WON=TRUE"),
            "golive":("GO_LIVE_DATE"," AND IS_WENT_LIVE=TRUE")}[t]
...
def sql_score_uc(t):
    ...
    return f"""
WITH base AS (
    SELECT USE_CASE_ACV, REGION, {_mk(de)} AS MONTH_KEY, {_qk(de)} AS QUARTER_KEY
...
base AS (
    ...
    FROM SALES.RAVEN.SDA_CLOSED_OPPORTUNITY_BOOKINGS_VIEW o
...
def sql_top_uc(t):
    de, we = _uc_params(t)
    pf = f"u.{de.split('::')[0]}::DATE" if "::" in de else f"u.{de}"
    return f"""
WITH {_SE_ATTR_CTE},
base AS (
    SELECT u.OWNER_NAME AS AE, COALESCE(se.SALES_ENGINEER_NAME,'—') AS SE,
           u.ACCOUNT_NAME, u.USE_CASE_NAME, u.USE_CASE_ACV, u.DM, u.REGION,
    ...
    WHERE u.THEATER='AMSExpansion'{we} AND {pf}>={_LOOKBACK}
...
WITH {_SE_ATTR_CTE},
acct_dim AS (
    SELECT SALESFORCE_ACCOUNT_ID, ACCOUNT_NAME, OPPORTUNITY_OWNER_NAME AS AE, DM, REGION
    FROM SALES.RAVEN.SDA_OPPORTUNITY_VIEW
    WHERE THEATER='AMSExpansion' AND DS={_OPP_MAX_DS}
